...
expert = ExpertSystem(json_path="data/json/dataset_vi.json")
logistic = LogisticPredictor(model_path="data/models/logistic_model.pkl")
...
with tab1:
    ...
    if st.button("Chẩn đoán", key="expert_diag"):
        if not selected_symptoms:
            ...
            new_syms = expert.extract_symptoms(symptom_text, st.session_state.facts)
            if isinstance(new_syms, set):
                for sym, val in new_syms:
                    st.session_state.facts[sym] = val

            ranked = expert.forward_chaining(st.session_state.facts)
            ...
            if not ranked:
                ...
                # Cảnh báo nếu độ tin cậy thấp hơn ngưỡng
                if top["percent"] < threshold:
                    st.warning("⚠️ Kết quả có độ tin cậy thấp do thiếu triệu chứng. Vui lòng bổ sung thêm.")

                # Lưu lịch sử
                st.session_state.history.append(
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} – {top['name']} ({top['percent']:.1f}%)"
                )
